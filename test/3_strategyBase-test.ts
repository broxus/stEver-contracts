...
describe("Strategy base", function () {
  ...
  it("strategy and vault state should be changed after report", async () => {
    ...
    const strategyReportEvent = traceTree!.findForContract({ contract: vault.vaultContract, name: "StrategyReported" });

    expect(strategyInfoAfter.totalAssets).to.be.equals(
      new BigNumber(strategyReportEvent[0]!.params!.report.totalAssets).minus(toNano("0.3")).toString(),
    );
  ...
  it("should strategy request value from vault", async () => {
    ...
    await newStrategy.emitDePoolRoundComplete(locklift.utils.toNano(10));
  });
